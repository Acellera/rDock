...
namespace Rbt
{
  ...
  public:
    ...
    RbtBool operator()(const RbtAtom *pAtom) const { return bIsCationic(pAtom) && bIsCarbon(pAtom) && bIsPi(pAtom) && !bIsCyclic(pAtom); }
  };
  ...
  public:
    ...
    explicit isCoordinationNumber_eq(RbtInt nn) : n(nn), eCNType(TOTAL), atNo(0), ffType(""), hybrid(RbtAtom::UNDEFINED) {}
    // Atomic number coordination number
    explicit isCoordinationNumber_eq(RbtInt nn, RbtInt nAt) : n(nn), eCNType(ATNO), atNo(nAt), ffType(""), hybrid(RbtAtom::UNDEFINED) {}
    // Force field type coordination number
    explicit isCoordinationNumber_eq(RbtInt nn, const RbtString &strType) : n(nn), eCNType(FFTYPE), atNo(0), ffType(strType), hybrid(RbtAtom::UNDEFINED) {}
    // Hybridisation state coordination number
    explicit isCoordinationNumber_eq(RbtInt nn, RbtAtom::eHybridState eState) : n(nn), eCNType(HYBRID), atNo(0), ffType(""), hybrid(eState) {}
    RbtBool operator()(RbtAtom *pAtom) const
    {
      switch (eCNType)
      {
  ...
  RbtUInt GetNumAtoms(const RbtAtomList &atomList, const Predicate &pred)
  {
