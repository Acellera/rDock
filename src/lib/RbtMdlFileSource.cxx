...
RbtMdlFileSource::RbtMdlFileSource(const RbtString &fileName, RbtBool bPosIonisable, RbtBool bNegIonisable, RbtBool bImplHydrogens) : RbtBaseMolecularFileSource(fileName, IDS_MDL_RECDELIM, "MDL_FILE_SOURCE"), // Call base class constructor
  ...
  if (!m_bParsedOK)
  {
    ...
    try
    {
      ...
      while ((m_bondList.size() < nBondRec) && (fileIter != fileEnd))
      {
        ...
        if ((idxAtom1 > nAtomRec) || (idxAtom2 > nAtomRec))
