...
ref = [
    item2.split() for item2 in [item.rstrip() for item in open(sys.argv[1]).readlines()]
]
test = [
    item2.split() for item2 in [item.rstrip() for item in open(sys.argv[2]).readlines()]
]
...
if error == 1:
    print(
        "The test failed, please check the compilation is OK and no errors were raised."
    )
else:
    print(
